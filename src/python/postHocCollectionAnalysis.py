...
query_fetch_event = (
    "SELECT Name, StartTime, StopTime "
    "FROM Event "
    "WHERE ID = %(Event_ID)s")

query_fetch_events_tweets = (
    "SELECT Tweet_ID, Text "
    "FROM Tweet "
    "JOIN TweetInEvent TinE "
    "ON TinE.Tweet_ID = Tweet.ID "
    "    AND TinE.Event_ID = %(Event_ID)s "
    "WHERE Tweet.Timestamp >= %(StartTime)s "
    "    AND Tweet.Timestamp < %(StopTime)s "
    "ORDER BY Tweet.Timestamp ")

query_push_tweet_distinction = (
    "UPDATE Tweet "
    "SET `Distinct` = %(Distinct)s "
    "WHERE ID = %(Tweet_ID)s ")
...
def main():
    ...
    parser.add_argument("-e", "--event_id", nargs="+", required=False,
                        help='Which collection(s) to work on')
...
def recalculateDistinct():
    download = dbDownload.cursor()
    upload = dbUpload.cursor()
    ...
    download.execute(query_fetch_event, query_fields)
    event = download.fetchone()
    
    print ('Looking for Tweets\n')
    
    query_fields['StartTime'] = datetime.strftime(event[1], '%Y-%m-%d %H:%M:%S')
    query_fields['StopTime'] = datetime.strftime(event[2], '%Y-%m-%d %H:%M:%S')
    download.execute(query_fetch_events_tweets, query_fields)
    ...
    for tweet in dbIterator(download): 
        # Iterate through the tweets, getting the distinct ones
        text = tweet[1]
        ...
        query_fields['Tweet_ID'] = tweet[0]
        query_fields['Distinct'] = distinct

        upload.execute(query_push_tweet_distinction, query_fields)
        
        # Every so often update the user on the progress
        if(n_tweets % n_report == 0):
            print ('\t ' + str(n_tweets) + ' tweets \t' + str(n_distinct) + ' distinct')
...
def connectToServer():
    ...
    with open(options.config) as config_file:
        ...
        if(options.database):
            ...
            dbDownload = mysql.connector.connect(
                user=config["storage"]["user"],
                password=config["storage"]["password"],
                host=config["storage"]["host"],
                database=config["storage"]["database"]
            )
            dbUpload = mysql.connector.connect(
                user=config["storage"]["user"],
                password=config["storage"]["password"],
                host=config["storage"]["host"],
                database=config["storage"]["database"]
